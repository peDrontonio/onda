...
public:
  DisturbanceApplier() : Node("disturbance_applier_node"), disturbance_active_(false), marker_id_(0) {
    ...
    this->declare_parameter<std::string>("disturbance_link", "manipulator::wrist_3_link");
    this->declare_parameter<std::string>("reference_frame", "world");
    this->declare_parameter<double>("force_duration", 100.0);
    
    link_name_ = this->get_parameter("disturbance_link").as_string();
    reference_frame_ = this->get_parameter("reference_frame").as_string();
    force_duration_ = this->get_parameter("force_duration").as_double();
    ...
    marker_pub_ = this->create_publisher<visualization_msgs::msg::Marker>("/disturbance_marker", 1);
  ...
  void disturbanceCallback(
    const std::shared_ptr<manipulator_msgs::srv::ApplyDisturbance::Request> req,
    std::shared_ptr<manipulator_msgs::srv::ApplyDisturbance::Response> res
  ) {
  ...
  bool applyForceInGazebo(const geometry_msgs::msg::Vector3& force) {
    ...
    request->start_time = rclcpp::Time(0);  // Apply immediately
    request->duration = rclcpp::Duration::from_seconds(force_duration_);
    
    if (!gazebo_wrench_client_->wait_for_service(std::chrono::seconds(1))) {
      RCLCPP_ERROR(this->get_logger(), "Gazebo ApplyBodyWrench service not available");
      return false;
    }
    
    auto result = gazebo_wrench_client_->async_send_request(request);
...
int main(int argc, char** argv) {
  ...
  auto applier = std::make_shared<DisturbanceApplier>();
  
  RCLCPP_INFO(applier->get_logger(), "Disturbance Applier Node running... Press Ctrl+C to exit");
  rclcpp::spin(applier);
  rclcpp::shutdown();
